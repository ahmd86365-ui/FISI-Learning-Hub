...
import { BarChart3, BookmarkCheck, CircleAlert, Flame, Menu, Search, UserRound, UserRoundX, LogIn, Languages } from 'lucide-react'
      ...
      <header className="sticky top-0 z-40 border-b border-ink-200/80 bg-white/80 backdrop-blur-lg dark:border-ink-800/80 dark:bg-ink-950/80">
        <div className="mx-auto flex h-16 max-w-content items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <IconButton
              icon={<Languages className="h-[1.15rem] w-[1.15rem]" />}
              label={translationEnabled ? 'Arabische Wortübersetzung deaktivieren' : 'Arabische Wortübersetzung aktivieren'}
              onClick={toggleTranslation}
              className={translationEnabled ? 'bg-brand-50 text-brand-700 dark:bg-brand-500/10 dark:text-brand-400' : ''}
            />
